...
function setPassword() {
  ...
  if (
    confirm(
      "Are you sure you want to password protect this page? If you do, you will not be able to access this page without the password. If you do not want to password protect this page, click cancel."
    ) == true
  ) {
...
if (localStorage.getItem("analytics") != "off") {
  ...
  scriptTagGTAG.setAttribute(
    "src",
    "https://www.googletagmanager.com/gtag/js?id=G-CX3B4NHEG0"
  );
...
function getSearchEngineURL() {
  return (
    localStorage.getItem("searchEngineURL") || "https://google.com/search?q="
  );
}
...
function setupCustomShortcut() {
  if (localStorage.getItem("shortcutURL") != null) {
    document.getElementById("customShortcutIcon").innerHTML =
      localStorage.getItem("shortcutLogo");
    document.getElementById("customShortcutDiv").onclick = function () {
      openURL(localStorage.getItem("shortcutURL"));
    };
  }
}
  ...
  fetch("./assets/announcement.json")
    ...
    .then((data) => {
      ...
      const randomAnnouncement =
        announcementText[Math.floor(Math.random() * announcementText.length)];
